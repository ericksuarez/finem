...
<!--
<input type="submit" value="Terminar" class="btn btn-info pull-right"/>
-->
<span class="pull-right" style="height: 5px; width: 5px;"></span>
<input type="submit" value="Guardar" class="btn btn-success pull-right"/>
<div class='clearfix'></div>
<?php echo form_close(); ?>
<script type="text/javascript">
    function hide(datos_laborales) {
        var item = document.getElementById(datos_laborales);
        if (item) {
            item.className='oculto';
        }
    }
    function unhide(datos_laborales) {
        var item = document.getElementById(datos_laborales);
        if (item) {
    ...
    $(document).ready(function(){
        ...
        $('#uni').change(function(){
            ...
            if(opcion != '') {
                ...
                $('#carreras').html('<select id="especialidad"><option value="">Selecciona una carrera</option></select>');
        ...
        $('#campus').change(function() {
            ...
            if(opcion != '') {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('campus/ajax_carrera');?>",
                    cache: false,
                    data: {campus: opcion}
                }).done(function( html ) {
                    $("#error_ajax").css('display','none');
                    ...
                    alert(textStatus);
                    $("#error_ajax").html('<p>Error al poblar las carreras.</p>');
                    $("#error_ajax").css('display','block');
                    $("#results").html("<center><i class='icon icon-frown' style='font-size: 20em;'><br /><span style='font-size: 16px;'>No hemos encontrado resultados.</span></i></center>");
                    //alert( "Falla en el sistema. Contacte a su administrador.");
                });
